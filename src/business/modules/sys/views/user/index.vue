    ...
    <d2-container :class="{'page-compact':crud.pageOptions.compact}">
        ...
        <d2-crud-x
          ...
          <div slot="header">
            ...
            <el-button slot="header" v-permission="'sys:user:add'" size="small" type="primary" @click="addRow"><i class="el-icon-plus"/> 新增</el-button>
        ...
        <el-dialog title="授予角色"
          ...
          <el-checkbox-group v-model="checkedRoleIds">
            <el-checkbox class="d2-mb-10" v-for="option in roleList" :key="option.roleId"  :label="option.roleId" >{{option.roleName}}</el-checkbox>
          </el-checkbox-group>
...
export default {
  ...
  methods: {
    ...
    authzHandle (event) {
      roleApi.GetAllList().then(ret => {
        const roleList = ret.data
        let userRoles = event.row.roleIds
        if (userRoles == null) {
          userRoles = []
        }
        const checkedRoleIds = roleList.map(item => item.roleId).filter(item => userRoles.indexOf(item) >= 0)
