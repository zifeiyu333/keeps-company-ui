    ...
    <d2-container :class="{'page-compact':crud.pageOptions.compact}">
        <!-- <template slot="header">角色管理
          <example-helper title="权限管理帮助" >
            <div>
              ...
              <link-button href="https://keeps.xin">权限管理帮助文档</link-button>
            </div>
          </example-helper>
        </template> -->
        <d2-crud-x
        ...
        >
          ...
          <div slot="header">
            ...
            <el-button v-permission="'sys:role:add'" size="small" type="primary" @click="addRow"><i class="el-icon-plus"/> 新增</el-button>
...
import * as resourceApi from '../menu/api'
export default {
  name: 'Role',
  ...
  methods: {
    ...
    getAllCheckedLeafNodeId (tree, checkedIds, temp) {
      for (let i = 0; i < tree.length; i++) {
        ...
        if (item.children && item.children.length !== 0) {
          ...
          if (checkedIds.indexOf(item.menuId + '') !== -1) {
    ...
    authzHandle (event) {
      resourceApi.GetAllTree().then(ret => {
        this.$set(this, 'treeData', ret.data)
        this.$set(this, 'checkedKeys', [])
        // this.treeData = ret.data
        this.roleId = event.row.roleId
        return this.updateChecked(event.row.roleId)
      }).then(() => {
        this.dialogPermissionVisible = true
      })
    },
    updateChecked (id) {
      return api.QueryMenuIdList(id).then(ret => {
