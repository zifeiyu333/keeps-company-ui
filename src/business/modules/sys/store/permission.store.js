...
function formatRouter (parent, list) {
  ...
  list.forEach((item) => {
    ...
    if (item.menuType !== 2 && !StringUtils.isEmpty(item.routeComponent)) { // 如果是按钮 或者没有配置component，则不加入路由
      let component = null
      if (item.routeComponent === 'layoutHeaderAside') {
        component = layoutHeaderAside
      } else {
        component = () => import('@/business/modules' + item.routeComponent)
      }
...
function formatMenu (menuTree) {
  ...
  menuTree.forEach((item) => {
    if (item.menuType !== 1 || item.visible === 2) { // 只有菜单类型才加入
      return
    }
    let children
    if (item.children != null && item.children.length > 0) {
    ...
    menus.push({ path: item.routePath, title: item.menuName, icon: icon, children: children })
  })
  if (menus.length === 0) {
    menus = undefined
  }
...
function formatPermissions (menuTree, permissionList) {
  ...
  menuTree.forEach((item) => {
    if (item.perms != null && item.perms !== '') { // 权限为空
      permissionList.push(item.perms)
    }
    if (item.children != null && item.children.length > 0) {
