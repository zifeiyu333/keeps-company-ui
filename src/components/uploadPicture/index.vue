            ...
            <div class="mr20 mb20">
              <el-button-group>
                ...
                <el-tooltip class="item" effect="dark" content="添加分类" placement="top-start">
                  <el-button icon="el-icon-circle-plus-outline" style="font-size: 15px;" size="mini" @click="onAdd('0')"></el-button>
                </el-tooltip>
          ...
          <div class="pictrueList acea-row">
            ...
            <div class="conters scrollbarAll" :style="{maxHeight:(!pictureType?'296px':'700px')}">
              <div
                ...
                class="gridPic"
              >
                <img v-lazy="item.fileUrl" :class="item.isSelect ? 'on': '' " @click="changImage(item, index)" />
              </div>
            </div>
          </div>
        </div>
    ...
    >
      <el-form ref="editPram" :model="editPram" label-width="100px"  v-loading="loading">
        ...
        <el-form-item label="排序">
          <el-input-number v-model="editPram.orderNum"/>
        </el-form-item>
        <el-form-item>
...
import * as ossApi from '@/api/sysOssFile'
...
export default {
  ...
  data () {
    return {
      ...
      classifyId: 0,
      myHeaders: { Authorization: util.cookies.get('token'), appid: process.env.VUE_APP_ID },
      treeFrom: {
        status: 1,
        type: 2
      },
  ...
  methods: {
    ...
    handlerSubmit (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.editPram.id === '0') {
            this.editPram.id = ''
            api.AddObj(this.editPram).then(data => {
              this.$message.success('创建成功')
              this.visible = false
              this.getList()
            })
          } else {
            api.UpdateObj(this.editPram).then(data => {
    ...
    getList () {
      ...
      api.GetTree(this.treeFrom).then(res => {
    ...
    onEdit (id) {
      ...
      api.GetObj(id).then((res) => {
        this.editPram = res.data
        this.editPram.pid = this.editPram.pid + ''
        this.loading = false
      })
      this.visible = true
    },
    // 删除
    handleDelete (id) {
      this.$confirm('确定要删除该数据吗？', '询问', {
        type: 'warning'
      }).then(() => {
        return api.DelObj(id).then(() => {
    ...
    handleUploadForm (param) {
      ...
      ossApi.upload(formData, data).then(res => {
        loading.close()
    ...
    // 文件列表
    getFileList () {
      this.tableData.groupId = this.tableData.pid
      ossApi.GetList(this.tableData).then(async res => {
        this.pictrueList.list = res.data.records
    ...
    checkPics () {
      ...
      if (this.$route && this.$route.query.field === 'dialog') {
        ...
        for (let i = 0; i < this.checkPicList.length; i++) {
          str += '<img src="' + this.checkPicList[i].fileUrl + '">'
        }
        /* eslint-disable */
    ...
    editPicList(tit) {
      ...
      this.$confirm('确定要删除该数据吗？', '询问', {
        ...
        return ossApi.DelObj(this.ids.join(',')).then(() => {
    ...
    getMove() {
      ossApi.OssMoveApi(this.sleOptions).then(async res => {
