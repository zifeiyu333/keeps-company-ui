...
new Vue({
  ...
  created () {
    ...
    const defaultIndex = defaultMenuIndex > menuHeader.length ? menuHeader.length : defaultMenuIndex
    if (menuHeader[defaultIndex]) {
      this.$store.commit('d2admin/menu/asideSet', menuHeader[defaultIndex].children)
    }
    // 初始化菜单搜索功能
